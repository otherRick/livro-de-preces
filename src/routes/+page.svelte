<script lang="ts">
	...
	onMount(async () => { try { await Promise.all([carregar('vivos'), carregar('mortos')]); await tick(); irPara(paginas.vivos.length - 1, false); } catch (e) { avisar(e instanceof Error ? e.message : 'Não foi possível abrir o livro agora.'); } finally { carregando = false; }
		...
		if (sitekey) { const script = document.createElement('script'); script.src = 'https://challenges.cloudflare.com/turnstile/v0/api.js?render=explicit'; script.async = true; script.onload = () => (window as typeof window & { turnstile?: { render: (element: string, options: { sitekey: string; appearance: 'interaction-only'; callback: (token: string) => void }) => void } }).turnstile?.render('#turnstile', { sitekey, appearance: 'interaction-only', callback: (token: string) => turnstileToken = token }); document.head.append(script); }
	});
	// Mantém a página do livro intacta quando o teclado virtual aparece. Em
	// navegadores Chromium usamos a altura precisa do VirtualKeyboard; no Safari
	// e nos demais, visualViewport informa quanto o teclado tomou da tela.
	onMount(() => {
		const viewport = window.visualViewport;
		const virtualKeyboard = (navigator as Navigator & { virtualKeyboard?: { overlaysContent: boolean; boundingRect: DOMRect; addEventListener: (tipo: 'geometrychange', ouvinte: () => void) => void; removeEventListener: (tipo: 'geometrychange', ouvinte: () => void) => void } }).virtualKeyboard;
		const definirAltura = (altura: number) => document.documentElement.style.setProperty('--teclado-virtual', `${Math.max(0, altura)}px`);
		const peloViewport = () => definirAltura(viewport ? window.innerHeight - viewport.height - viewport.offsetTop : 0);
		const peloTeclado = () => definirAltura(virtualKeyboard?.boundingRect.height ?? 0);

		if (virtualKeyboard) { virtualKeyboard.overlaysContent = true; virtualKeyboard.addEventListener('geometrychange', peloTeclado); peloTeclado(); }
		else { viewport?.addEventListener('resize', peloViewport); viewport?.addEventListener('scroll', peloViewport); peloViewport(); }

		return () => { if (virtualKeyboard) virtualKeyboard.removeEventListener('geometrychange', peloTeclado); else { viewport?.removeEventListener('resize', peloViewport); viewport?.removeEventListener('scroll', peloViewport); } definirAltura(0); };
	});
</script>
